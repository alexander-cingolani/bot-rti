...
from bot.app.components.conversations.protest_creation import protest_creation
...
async def announce_protests(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if rnd := championship.protesting_round():
        text = (
...
async def close_protest_window(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if championship:
        if rnd := championship.protesting_round():
            if not rnd.protests:
                await context.bot.send_message(
                    chat_id=config.PROTEST_CHANNEL, text="Nessuna segnalazione ricevuta."
                )
