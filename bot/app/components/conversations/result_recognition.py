...
async def results_input_entry_point(
    ...
    # If no championship was returned it means that there are no active championships.
    if not championship:
        await update.message.reply_text(
...
async def ask_session(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for session in current_round.sessions:
        ...
        if not user_data.get(session):
            user_data[session] = {"results": [], "fastest_lap_drivers": []}
        else:
            completed_sessions += 1

    chunked_session_buttons = list(chunked(session_buttons, 3))
    text = "Scegli la sessione dove vuoi inserire i risultati."

    if completed_sessions == len(current_round.sessions):
        chunked_session_buttons.append([InlineKeyboardButton("Salva Risultati")])
        text = "Puoi controllare i risultati o salvarli."

    reply_markup = InlineKeyboardMarkup(completed_sessions)
...
async def ask_qualifying_results(
    ...
    """Asks the user for the qualifying results screenshot."""
...
async def download_quali_results(
    ...
    await update.effective_chat.send_chat_action(ChatAction.TYPING)  # type: ignore

    # Saves the photo/text containing the qualifying results.
    if update.message:
        if update.message.text:
            ...
            try:
                ...
                # If the user sent an ivalid file format he is told to try again.
            ...
            success, user_data["quali_results"] = image_to_results(
                "results.jpg", drivers
            )
...
async def download_race_1_results(
    ...
    if update.message:
        if update.message.text:
            ...
            _, results = image_to_results(screenshot, drivers)
    ...
    for result in results:
        if result.seconds is None:
            ...
            racetime = seconds_to_text(result.seconds)
        driver = result.driver.driver.psn_id if result.driver else "NON_RICONOSCIUTO"
        text += f"\n{driver} {racetime}"
...
async def ask_fastest_lap_1(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    results = cast(
        list[Result], user_data["race_results"][championship_round.long_race]["results"]
    )

    for i, result in enumerate(results):
        driver = result.driver
        participated = False if result.seconds is None else True
        if driver.car_class == car_class and participated:
...
async def ask_2nd_fastest_lap_1(
    ...
    results = cast(
        list[Result],
        user_data["race_results"][championship_round.sprint_race]["results"],
    )
    for i, result in enumerate(results):
        driver = result.driver
        participated = False if result.seconds is None else True
        if driver.current_class() == car_class and participated:
            buttons.append(InlineKeyboardButton(driver.psn_id, callback_data=f"d{i}"))
    ...
    for class_results in individual_class_results.values():
        ...
        for pos, quali_res in enumerate(class_results, start=1):
            if quali_res.seconds:
                gap_to_first = quali_res.seconds - class_pole_time  # type: ignore
